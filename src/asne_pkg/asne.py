...
class ASNE:
    ...
    def _setup_variables(self):
        ...
        self.node_features = tf.compat.v1.sparse_placeholder(tf.float32,
                                                   shape=[None, self.feature_count])

        self.right_nodes = tf.compat.v1.placeholder(tf.int32,
                                          shape=[None, 1])
    ...
    def try_model(self):
        ...
        print(self.noise_embedding)
        em = self.noise_embedding.numpy()
        #tf.io.write_file("temp_embedding.txt", self.combined_embed)
        run_embedded_training(em)
        return 0.001
